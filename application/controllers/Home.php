...
class Home extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if($this->input->post('lvalue')) {             $this->homequickQuote();           }
        $data = common_data();
    ...
    { //Load Product Category Page
        ...
        { //load single product description page
			...
			/* START MEta update  */
				...
				$data['product']['meta_title']=$data['product']['name'] ." | ";
				}
				...
				$data['product']['meta_description']= $data['product']['name'] . " ; " .trim($m_desc)."Shop Online at labosearch.com!";
        ...
        if($this->formval->run('get_quote_validation') != false) {
            ...
            $msg_body = '<style type="text/css"></style>
                ...
                <div class="table-responsive">
                    <h1 style="text-align:center; font-size:20px ;">Quotation From <a style="color:#0b5387;" href="<?php echo base_url(); ?>">labosearch.com</a></h1>
                    <br>
                    <table class="table">
                        <tr>
                            <td>
                        ...
                        <tr>
                            <td>
                                <h3><b>Product Name : </b>' . $product . '</h3>
                            </td>
                        </tr>
                        <tr>
                            <td>
            ...
            $headSubject = 'Quotation From - www.labosearch.com';
            $to = EMAIL_WEBSITE;
           // $cc = EMAIL_WEBSITE_CC;
            $from = $email;
    ...
    public function enquiry()//Footer enquiry
    {
        ...
        if ($this->formval->run('enquiry_validation') != false) {
            ...
            $msg_body = '<style type="text/css">
                ...
                <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" media="all">
                <div class="table-responsive">
                    <h1 style="text-align:center; font-size:20px">Enquiry From <a style="color:#0b5387;" href="<?php echo base_url(); ?>">labosearch.com</a></h1>
                    <br>
                    <table class="table">
                        <tr>
                            <td>
    ...
    function homequickQuote()//header Inquiry
    {
        if(!empty($this->input->post('captcha4'))){
            if($this->session->userdata('captcha')!=$this->input->post('captcha4')){
                $this->session->set_flashdata('pd_quote_error', 'Entered captcha code does not match! Kindly try again.');
                redirect(base_url());
            }
        }
        if ($this->formval->run('homequick_quote_validation') != FALSE) {
            $data = $this->input->post();
            // print_r($data); exit;
            $name = $data['lname'];
            $email = $data['lemail'];
            $product = $data['lproduct_name'];
            // $location = $data['llocation'];
            // $url = $data['lurl'];
            // $message = $data['lmessage'];
            $msg_body  = '<style type="text/css"> 
                        </style>
                        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" media="all">
                        <div class="table-responsive">
                            <h1 style="text-align:center;">Inquiry From <a style="color:#359694;" href="www.labosearch.com">labosearch.com</a></h1>
                            <br>
                            <table class="table ">
                                <tr>
                                    <td>
                                ...
                                <tr>    
                                    <td>
                                        <b>Email : </b><a style="color:#359694;" href="mailto:' . $email . '">' . $email . '</a>
                                    </td>
                                </tr> 
                                <tr>    
                                    <td>  
    ...
    {
        $data = array();
        $meta_title = 'Thank you | Get Quote | labosearch.com Equipment Ltd';
        $meta_keyword = 'Thank you,Get Quote, labosearch.com Equipment Ltd';
        $meta_description = 'Thank you,Get Quote, labosearch.com Equipment Ltd';
        $thank_you = 6;
        /****** COMMON DATA  ****/
